...
public class ChannelDao extends BaseDao<Channel> implements IChannelDao {
	...
	public List<Channel> listByParent(Integer pid) {
		String hql ="select c from Channel c left join c.parent cp where cp.id="+pid+" order by c.orders";
	...
	public int getMaxOrderByParent(Integer pid) {
		...
		if(pid==null|| pid ==0) hql = "select max(c.orders) from Channel c where c.parent is null order by orders";
